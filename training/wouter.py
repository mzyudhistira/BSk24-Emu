...
def model_training(training_name, model, training_data, validation_data, batch, epoch, weights_file = '',
    ...
    model_checkpoint_callback = ModelCheckpoint(save_weights_only = True, save_best_only = True,
                                                monitor = 'val_mae', mode = 'min', filepath = best_weights)
    resource_monitor = ResourceUsageCallback()

    history = model.fit(feature, target, validation_data = (val_feature, val_target), batch_size=batch, epochs=epoch,
    ...
    performance_log = {'training_time':training_time, 'avg_cpu_usage': avg_cpu_usage, 'avg_gpu_usage': avg_gpu_usage}
...
def fine_grain_training(model, train_dat, val_dat, batch_number = [32, 16, 4], epoch_number = [1000, 500, 500], training_name=''):
    """
    ...
    if training_name == '':
        training_names = [f'test.batch={str(batch_number[i])}.epoch={epoch_number[i]}.stage{i+1}' for i in range(3)]

    else:
        training_names = [f'{training_name}.batch={str(batch_number[i])}.epoch={epoch_number[i]}.stage{i+1}' for i in range(3)]

    performance_logs = [i for i in range(3)]
    history = [i for i in range(3)]

    history[0], lastfname, bestfname, performance_logs[0] = model_training(training_names[0], model, train_dat, val_dat, 
                                                                          batch_number[0], epoch_number[0])

    history[1], lastfname, bestfname, performance_logs[1] = model_training(training_names[1], model, train_dat, val_dat, 
                                                                          batch_number[1], epoch_number[1], weights_file=lastfname)

    model = wouter_model(len(train_dat[0][:-1]),'adagrad')

    history[2], lastfname, bestfname, performance_logs[2] = model_training(training_names[2], model, train_dat, val_dat, 
                                                                          batch_number[2], epoch_number[2], weights_file=bestfname)
